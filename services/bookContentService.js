...
const getBookContentById = async (bookId, currentPage, contentId) => {
  // const book = data.book.find((b) => b.id == bookId);

  const { data: book } = await supabase.from("Book").select("*").eq("id", bookId);

  if (!book) {
    return { error: "Cannot Find book" };
  }
  const { data: content } = await supabase.from("BookContent").select("*").eq("bookId", bookId);
  ...
  let currentPageIndex = -1;
  if (contentId) {
    const entryWithContentId = content.find((entry) => entry.page_no == contentId);
    if (!entryWithContentId) {
      return { error: "Content ID not found" };
    }
    currentPageIndex = content.indexOf(entryWithContentId);
  ...
  const compressedContent = contentInRange.map((entry) => ({
    id: entry.id,
    bookId: entry.bookId,
    content: zlib.inflateSync(Buffer.from(entry.content, "base64")).toString("utf8"),
    // content: entry.content,
    page_no: entry.page_no,
  }));
